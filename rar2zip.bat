...
rem Scoopパスを環境変数から動的に決定
if defined SCOOP (
    set "SCOOP_7ZIP=%SCOOP%\apps\7zip\current\7z.exe"
) else (
    set "SCOOP_7ZIP=%USERPROFILE%\scoop\apps\7zip\current\7z.exe"
)
...
if "%~1"=="" (
    ...
    echo   - 削除を有効にする場合は DELETE_ORIGINAL=1 に変更
...
rem ================================================================
rem  フォルダ内RAR検索（for /r の遅延展開問題を回避）
rem ================================================================
:search_rar_in_folder
...
rem ZIP作成（無圧縮・最高速度）
echo [1/!TOTAL_STEPS!] ZIP作成中...（無圧縮・最高速度）
...
rem ZIP整合性チェック
echo [2/!TOTAL_STEPS!] ZIP整合性チェック中...
...
rem 一時フォルダの重複を安全に回避（ループで空きを確認）
set "TMP_BASE=!BASE!_tmp_unpack"
set "TMP=!TMP_BASE!"
:make_tmp
if exist "!TMP!" (
    set "TMP=!TMP_BASE!_%RANDOM%%RANDOM%"
    goto make_tmp
)
...
dir /b /a "!TMP!" >nul 2>&1
if errorlevel 1 (
    echo [エラー] RAR展開後のフォルダが空です。破損ファイルの可能性があります: "!SRC!"
    rd /s /q "!TMP!" >nul 2>&1
    goto convert_end
)
...
rem ZIP作成（最高速設定）
echo [2/!TOTAL_STEPS!] ZIP作成中...（無圧縮・最高速度）
...
rem ZIP整合性チェック
echo [3/!TOTAL_STEPS!] ZIP整合性チェック中...
"!SEVENZIP!" t "!ZIP!" >nul 2>&1
if errorlevel 1 (
...
if "!IS_SPLIT!"=="1" (
    ...
    rem ベース名から .partN（任意桁数）を除去
    set "BASE_ROOT=!NAME_NOEXT!"
    rem part + 数字列を末尾から除去（.part1, .part01, .part001 等に対応）
    for /f "delims=" %%B in ('echo !NAME_NOEXT!^| powershell -NoProfile -Command "$input.Trim() -replace '\.part\d+$',''"') do set "BASE_ROOT=%%B"

    echo 分割RARファイルを削除中: "!DIR!!BASE_ROOT!.part*.rar"

    rem 起点ファイル（part1）を削除
    call :safe_delete "!SRC!"
